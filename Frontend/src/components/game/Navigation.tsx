...
export const Navigation = ({ currentPage, onNavigate }: NavigationProps) => {
  return (
    ...
    <div className="fixed bottom-3 sm:bottom-8 left-1/2 -translate-x-1/2 bg-black/40 backdrop-blur-xl border border-white/10 text-white rounded-full px-2 sm:px-6 py-1.5 sm:py-2 shadow-2xl z-50 flex items-center gap-0.5 sm:gap-1">
      {navItems.map((item) => {
        ...
        return (
          <button
            ...
            className={cn(
              "relative flex flex-col items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-full transition-all duration-300",
              // Active: Bright Green Icon
              isActive ? "text-primary -translate-y-1" : "text-white/40 hover:text-white hover:bg-white/5"
            )}
          >
            <item.icon className={cn("w-5 h-5 sm:w-6 sm:h-6", isActive && "fill-current drop-shadow-[0_0_8px_rgba(52,211,153,0.5)]")} />
