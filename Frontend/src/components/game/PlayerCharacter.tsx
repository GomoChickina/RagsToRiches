...
export const PlayerCharacterComponent = ({
    ...
    if (mode === 'thinking') {
      return (
        <g>
          ...
          />
          <motion.circle cx="140" cy="42" r="3" fill="white" opacity="0.6" animate={{ opacity: [0.3, 0.8, 0.3], scale: [0.8, 1, 0.8] }} transition={{ duration: 1.5, repeat: Infinity, delay: 0 }} />
          <motion.circle cx="152" cy="30" r="5" fill="white" opacity="0.6" animate={{ opacity: [0.3, 0.8, 0.3], scale: [0.8, 1, 0.8] }} transition={{ duration: 1.5, repeat: Infinity, delay: 0.3 }} />
          <motion.circle cx="165" cy="16" r="7" fill="white" opacity="0.6" animate={{ opacity: [0.3, 0.8, 0.3], scale: [0.8, 1, 0.8] }} transition={{ duration: 1.5, repeat: Infinity, delay: 0.6 }} />
        </g>
      );
    }
    return (
  ...
  const renderMouth = (mode: string) => {
    if (mode === 'happy') return (
      <g>
        ...
        <motion.path d="M 94 93 Q 100 99 106 93" fill="#f87171" animate={{ scaleY: [1, 1.3, 1], y: [0, 2, 0] }} transition={{ duration: 0.4, repeat: Infinity }} />
      </g>
    );
    if (mode === 'sad') return <path d="M 85 92 Q 100 80 115 92" stroke={c.black} strokeWidth="3.5" strokeLinecap="round" fill="none" />;
  ...
  const renderSweat = (mode: string) => {
    ...
    return <motion.path d="M 55 50 Q 50 58 55 66 Q 60 58 55 50" fill="#60a5fa" animate={{ y: [0, 8, 0], opacity: [0, 0.8, 0] }} transition={{ duration: 2, repeat: Infinity, ease: 'easeIn' as const }} />;
  };
  ...
  const renderStars = (mode: string) => {
    ...
    return (
      <g>
        {[{ cx: 35, cy: 40, delay: 0 }, { cx: 165, cy: 35, delay: 0.2 }, { cx: 25, cy: 75, delay: 0.4 }, { cx: 178, cy: 70, delay: 0.15 }].map((s, i) => (
          <motion.text key={i} x={s.cx} y={s.cy} textAnchor="middle" fontSize="16" animate={{ scale: [0, 1.4, 0], rotate: [0, 180, 360], opacity: [0, 1, 0] }} transition={{ duration: 0.8, repeat: Infinity, delay: s.delay }} style={{ transformBox: 'fill-box', transformOrigin: 'center' }}>⭐</motion.text>
        ))}
      </g>
    );
  };
  ...
  const renderAvatar = (mode: string) => (
    <motion.svg viewBox="-25 -10 250 270" className="w-full h-full drop-shadow-xl" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ overflow: 'visible' }}>
      {renderStars(mode)}

      {/* STICK LIMBS */}
      <motion.path stroke={c.black} strokeWidth="7" strokeLinecap="round" variants={leftLegAnim} animate={mode} initial={false} />
      <motion.path stroke={c.black} strokeWidth="7" strokeLinecap="round" variants={rightLegAnim} animate={mode} initial={false} />
      <motion.path stroke={c.black} strokeWidth="6.5" strokeLinecap="round" variants={leftArmAnim} animate={mode} initial={false} />
      <motion.path stroke={c.black} strokeWidth="6.5" strokeLinecap="round" variants={rightArmAnim} animate={mode} initial={false} />

      {/* BODY */}
      <motion.g variants={bodyAnim} animate={mode} initial={false}>
        <rect
          x="65" y="110" width="70" height="80"
          fill={outfitId === 'business_suit' ? c.suit : outfitId === 'cool_hoodie' ? c.shirtHoodie : c.shirtDefault}
          stroke={c.black} strokeWidth="3.5" rx="6"
        />
        ...
        {outfitId === 'cool_hoodie' && (
          <g>
            <line x1="88" y1="110" x2="88" y2="135" stroke="white" strokeWidth="2.5" strokeLinecap="round" opacity="0.8" />
            <line x1="112" y1="110" x2="112" y2="135" stroke="white" strokeWidth="2.5" strokeLinecap="round" opacity="0.8" />
            <path d="M 75 180 L 85 155 L 115 155 L 125 180 Z" fill="white" opacity="0.15" />
          </g>
        )}
        ...
        {/* HEAD */}
        <motion.g variants={headAnim} animate={mode} initial={false} style={{ transformBox: 'fill-box', transformOrigin: '100px 70px' }}>
          ...
          {hatId === 'grad_cap' && (
            <g>
              <path d="M 70 45 Q 100 25 130 45 Z" fill={c.black} stroke={c.black} strokeWidth="2" />
              <path d="M 100 25 L 135 60" stroke={c.gold} strokeWidth="2.5" />
              <circle cx="135" cy="62" r="3.5" fill={c.gold} />
              <polygon points="100,10 45,30 100,50 155,30" fill={c.black} stroke={c.black} strokeWidth="2" strokeLinejoin="round" />
            </g>
          )}
        </motion.g>
      </motion.g>
  ...
  return (
    <>
      <div className={`relative ${size === 'sm' ? 'w-36 h-44' : 'w-48 h-56'} overflow-visible`} style={{ visibility: reaction === 'happy' ? 'hidden' : 'visible' }}>
        {renderAvatar(reaction)}
      </div>

      <AnimatePresence>
        {reaction === 'happy' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-emerald-950/60 backdrop-blur-sm">
            <motion.div animate={{ rotate: 360 }} transition={{ duration: 6, repeat: Infinity, ease: 'linear' }} className="absolute inset-0 flex items-center justify-center pointer-events-none">
              {[...Array(8)].map((_, i) => (
                <motion.div key={i} className="absolute w-[600px] h-4 rounded-full opacity-20" style={{ background: `hsl(${i * 45}, 90%, 60%)`, rotate: `${i * 45}deg`, transformOrigin: 'center' }} animate={{ scaleX: [1, 1.3, 1] }} transition={{ duration: 0.8, repeat: Infinity, delay: i * 0.1 }} />
              ))}
            </motion.div>

            {[...Array(12)].map((_, i) => (
              <motion.div key={i} className="absolute w-3 h-3 rounded-sm" style={{ background: `hsl(${i * 30}, 80%, 60%)`, left: `${20 + (i % 6) * 12}%`, top: `20%` }} animate={{ y: [0, 300 + Math.random() * 200], x: [0, (Math.random() - 0.5) * 200], rotate: [0, 720], opacity: [1, 0] }} transition={{ duration: 1.5, repeat: Infinity, delay: i * 0.1, ease: 'easeIn' }} />
            ))}

            <motion.div initial={{ scale: 0.3, y: 120, rotate: -20 }} animate={{ scale: 2.2, y: 0, rotate: 0 }} exit={{ scale: 0, opacity: 0, rotate: 20 }} transition={{ type: 'spring', stiffness: 180, damping: 14 }} className="w-64 h-72">
              {renderAvatar('happy')}
            </motion.div>

            <motion.div initial={{ scale: 0, y: 60 }} animate={{ scale: [0, 1.3, 1], y: 200 }} className="absolute font-black text-5xl tracking-widest text-white drop-shadow-[0_4px_0_rgba(0,0,0,0.8)]" style={{ textShadow: '0 0 30px rgba(251,191,36,0.8)' }}>
              SMART MOVE! 🎉
            </motion.div>
          </motion.div>
        )}
